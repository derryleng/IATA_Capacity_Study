plot_ASMA <- function(metric, type, entity, breakdown=T, annual=F, top=10, fontsize=12, years, month) {
  ...
  if (metric == "Total Monthly Delays") {
    ...
    title <- paste("Total Monthly ASMA Additional Time for", entity)
    ...
    title <- paste("Average Monthly ASMA Additional Time Ranking", ifelse(type=="All Countries", "", paste("for", type)))
    ...
    title <- paste(month, "Average Monthly ASMA Additional Time Ranking", ifelse(type=="All Countries", "", paste("for", type)))
    ytitle <- "Average Delay (min.)"
    xtitle <- ""
    if (type == "All Countries") {
      temp <- dat$ASMA
