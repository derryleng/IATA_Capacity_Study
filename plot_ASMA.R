plot_ASMA <- function(metric, type, entity, breakdown=T, annual=F, top=10, fontsize=12, years, month) {
  ...
  if (metric == "Total Monthly Delays") {
    ...
    title <- paste("Average ASMA Additional Time Ranking")
    ytitle <- "Average Delay (min.)"
    xtitle <- ""

    temp <- subset(dat$ASMA_ANNUAL, NAME %in% Top20_Airports_ASMA_TAXI & YEAR %in% years) %>% .[rev(order(YEAR, TIME_ADD/FLIGHTS_UNIMPEDED))]
    ...
    temp <- subset(dat$ASMA, NAME %in% Top20_Airports_ASMA_TAXI & YEAR %in% years & MONTH %in% months[which(monthsfull == month)]) %>% .[rev(order(YEAR, TIME_ADD/FLIGHTS_UNIMPEDED))]
