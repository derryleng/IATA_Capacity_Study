  ...
  if (metric == "Delays per Flight") {
    ...
    if (breakdown == T & annual == T) {
      ...
      g <- g %>%
        ...
        add_lines(data=subset(dat$ATFM_ANNUAL, NAME %in% entity & YEAR %in% years[years != 2018]) %>% arrange(factor(YEAR, levels=years)), x=~YEAR, y=~FLIGHTS_TOTAL, name="Total Flights", line=list(color="rgb(213,16,103)"), yaxis="y2") %>%
